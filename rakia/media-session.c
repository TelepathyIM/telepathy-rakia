 ...
 */
static const char session_states[NUM_TPSIP_MEDIA_SESSION_STATES][16] =
{
    "created",
    "invi-sent",
    "invi-recv",
...
#define TPSIP_MEDIA_SESSION_GET_PRIVATE(session) ((session)->priv)
...
static void rakia_media_session_init (RakiaMediaSession *self)
{
  RakiaMediaSessionPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE ((self),
      TPSIP_TYPE_MEDIA_SESSION, RakiaMediaSessionPrivate);
                                ...
                                RakiaMediaSessionState new_state)
{
  ...
  switch (new_state)
    ...
    case TPSIP_MEDIA_SESSION_STATE_ENDED:
      ...
      break;
    case NUM_TPSIP_MEDIA_SESSION_STATES:
